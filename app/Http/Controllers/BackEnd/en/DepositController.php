...
class DepositController extends Controller
{
  ...
  {
    $this->middleware('adminAccess', ['except' => ['voguepayDepositNotify','upaycardDepositSuccess','upaycardDepositFail','skrillDepositStatus','perfectMoneyDepositStatus']]);
  }
...
public function localbankFunds(Request $request){
  ...
  $apc = DB::table('cms_liveaccount')->where('email',session('login_email'))->select('country')->first();
      $country_name=$apc->country;
       session(['country' => $country_name]);
       // Session::save();

  $country=DB::table('countries')->where('countries_name',session('country'))->select('countries_name','currency_iso_code')->first();
  // dd($country);
  $currency_iso = $country->currency_iso_code;
  
  return view ('backEnd.'.app()->getLocale().'.crm.deposit.local-bank-deposit',compact('accounts','selected_account','currency_iso'));
}


public function postlocalbankFunds(Request $request){

  // dd($request->amount);
  if(is_numeric($request->amount)==false){
    return redirect('/local-bank-transfer-funds')->withErrors(['Please Enter a valid amount']);
  }
  elseif($request->amount<1){
    return redirect('/local-bank-transfer-funds')->withErrors(['Minimum Bank deposit is 1 USD']);
  }
  else{
      $time = time(); 
      $apc = DB::table('cms_liveaccount')->where('email',session('login_email'))->select('affiliate_prom_code','country')->first();
      ...
      Session::save();
         ...
         if(Session::has('account_no') && Session::has('localbank')){
              ...
              Session::forget('localbank');

            Session::flash('msg','Your Deposit to '.$account_no.' is under process.');
              // return $account_no;
                 ...
                 $country=DB::table('countries')->where('countries_name',session('country'))->select('countries_name','currency_iso_code')->first();
                   // dd($country);
                    ...
                    // $user_mail='localbankdeposit@gicmarkets.com';
                    ...
                    Mail::send('backEnd.'.app()->getLocale().'.crm.mail.localbank-deposit-mail',
              ...
              // return redirect('/local-bank-transfer-funds')->withErrors(['Your deposit is under process.']);

              return redirect('/local-bank-transfer-funds');
            }
  ...
  public function upaycardDepositSuccess(){
      ...
      if($serverOutput['status']=='success' && $serverOutput['msg']=='Success' && $serverOutput['reference_id']==session('reference') && $serverOutput['order_id']==session('order_id') && $serverOutput['purchase_status']==9){
          ...
          $user_name=$general_information->company_name.' Finance';
...
public function skrillDepositTest(){
  // $merchant_id = '93307799';
  // $transaction_id = '50001538050354';
  ...
  // // $merchant_id = '4637827';
  // // $transaction_id = '5585262';
  // // $mb_secret = '327638C253A4637199CEBA6642371F20';
  // // $mb_amount = '9.99';
  // // $mb_currency = 'EUR';  
  // // $status = '2';
  // $md5sigCalc = strtoupper(md5($merchant_id.$transaction_id.$mb_secret.$mb_amount.$mb_currency.$status));
  // return $md5sigCalc;
  return view ('backEnd.'.app()->getLocale().'.crm.deposit.skrill-deposit-test');
...
public function postSkrillDeposit(Request $request){
  if(is_numeric($request->amount)==false){
    return redirect('/skrill_deposit')->withErrors(['Please Enter a valid amount']);
  }
  elseif($request->amount<1){
    return redirect('/skrill_deposit')->withErrors(['Minimum Skrill deposit is 1 USD']);
  }
  else{
    $time = time();      
    session(['account_no' => $request->deposit_to]);
...
public function skrillDepositCancel(){
  if(Session::has('account_no') && Session::has('skrill')){
    ...
    Session::forget('account_no');
    Session::forget('skrill');
    return redirect('/skrill_deposit')->withErrors(['Your Deposit has been cancelled. Please Retry.']);
...
public function postPerfectMoneyDeposit(Request $request){
  ...
  if(is_numeric($request->amount)==false){
    return redirect('/perfect_money_deposit')->withErrors(['Please Enter a valid amount']);
  }
  elseif($request->amount<1){
    return redirect('/perfect_money_deposit')->withErrors(['Minimum Perfect Money deposit is 1 USD']);
  }
  else{
    $time = time(); 
    $apc = DB::table('cms_liveaccount')->where('email',session('login_email'))->select('affiliate_prom_code')->first();
...
public function perfectMoneyDepositCancel(){
  if(Session::has('account_no') && Session::has('perfectMoney')){
    ...
    Session::forget('tr_time');
    Session::forget('perfectMoney');
    return redirect('/perfect_money_deposit')->withErrors(['Your Deposit has been cancelled. Please Retry.']);
...
public function postVoguepayDeposit(Request $request){
  if(is_numeric($request->amount)==false){
    return redirect('/voguepay_deposit')->withErrors(['Please Enter a valid amount']);
  }
  elseif($request->amount<1){
    return redirect('/voguepay_deposit')->withErrors(['Minimum Voguepay deposit is 1 USD']);
  }
  else{
    $time = time();
