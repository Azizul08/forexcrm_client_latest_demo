...
class AuthController extends Controller
{
	...
	public function getConfirmLiveAccountRegistration(Request $request)
	{
		...
		if ($customer) {
			...
			// verification successfull message
				...
				Session::flash('password','Your Email has been verified succesfully.');
				return redirect('/login');

			}
			else{
				Session::flash('password','Your account has been verified already. Please login and enjoy.');
				return redirect('/login');
			}
	...
	public function postResetPassword(Request $request){
        ...
        if(count($exist)==0){
			Session::flash('notExist','Your email is not registered with us or the token has been expired');
			return Redirect::back();
		}
		...
		Session::flash('success_title','Password Reset link has been sent');
		Session::flash('success_register','A password reset link has been sent to your Email. Please check your Email.');
	...
	public function getResetPasswordSuccessful(Request $request)
	{
		...
		if ($customer) {
			...
			CMS_Liveaccount::whereemail($email)->update(['remember_token' => $reset_token]);
			
			// verification successfull message

			Session::flash('success_title','Password has been Reset Successfully');
			Session::flash('success_register','Your Password has been reset succesfully. Your Login details has been sent to your email. Please Check Your Email.');
		...
		// }
			...
			Session::flash('password','Your email is not registered with us or the token has been expired');
...
$obj = json_decode($json);
	...
	public function postDemoAccountRegister(Request $request)
	{
		...
		if($info_user){
			Session::flash('email','A demo account has already been registerd with this email ');
			return Redirect::back()->withInput();
		}
        //return $request->all();
...
//$newAccount->dob = "";
        ...
        // sending mail
        	$email_adds = $this->email_adds();
			$from_email_id=$email_adds->noreply_email;
			//$sent_email="m.azizulcse@gmail.com";
			// $sent_email=$email_adds->support_email;
			$from_name=config('app.name');
			$mail_data=$this->mail_data();
		...
		Mail::send('backEnd.'.app()->getLocale().'.crm.mail.send_demo_account_registration_verification_mail', ['newAccount' => $newAccount, 'token' => $token,'account_type'=>$request->account_type,'leverage'=>$request->leverage,'amount'=>$request->amount,'mail_data'=>$mail_data], function ($message) use($newAccount,$from_email_id, $from_name) {
			$message->from($from_email_id, $from_name);
			$message->to($newAccount->email,$newAccount->fname." ".$newAccount->lname);
			$message->subject('Demo Account Verification');
			});
		// Mail::send('backEnd.'.app()->getLocale().'.crm.mail.send_demo_account_registration_verification_mail', ['newAccount' => $newAccount, 'token' => $token,'account_type'=>$request->account_type,'leverage'=>$request->leverage,'amount'=>$request->amount,'mail_data'=>$mail_data], function ($message) use($newAccount,$from_email_id, $from_name,$sent_email) {
		// 	$message->from($from_email_id, $from_name);
		// 	$message->to($sent_email, $newAccount->fname." ".$newAccount->lname);
		// 	$message->subject('Demo Account Verification');
		// }); 
		Session::flash('success_title','Account Created Successfully');
		Session::flash('success_register','A verification link has been sent to your mail. Please check and confirm your email to receive login details.');
	...
	public function successDemoAccountRegistration(Request $request)
	{
		...
		if ($customer) {
			...
			if ($customer->email_status == 0) {
				...
				Session::flash('success_title','Email Verified Successfully');
				Session::flash('success_register','Your Email has been verified succesfully. Your MT4 Login details has been sent to your email. Please Check Your Email.');
...
// Notifications
			...
			Mail::send('backEnd.'.app()->getLocale().'.crm.mail.success_demo_account_password_mail', ['intIds'=>$intIds,'mt4_login'=>$login_id,'mt4_password'=>$mt4_password,'investor_password'=>$investor_password,'server'=>$server,'mail_data'=>$mail_data], function ($message) use($customer,$from_email_id, $from_name) {
				$message->from($from_email_id, $from_name);
				$message->to($customer->email, $customer->fname." ".$customer->lname);
				$message->subject('Demo Account Verification Successful');
			}); 
...
// echo $a;exit;
    ...
    // 'balance'=>$deposit
			...
			Mail::send('backEnd.'.app()->getLocale().'.crm.mail.success_live_account_password_mail', ['customer' => $customer,'mt4_login'=>$login_id,'mt4_password'=>$mt4_password,'investor_password'=>$investor_password,'mail_data'=>$mail_data], function ($message) use($customer,$from_email_id, $from_name) {
				$message->from($from_email_id, $from_name);
				$message->to($customer->email, $customer->fname." ".$customer->lname);
				$message->subject('Account Verification Successful');
			}); 

            // verification successfull message

			Session::flash('success_title','Account Verified Successfully');

			Session::flash('success_register','Your Email has been verified succesfully. Your Login details has been sent to your email. Please Check Your Email.');
			return redirect('/success-registration');
