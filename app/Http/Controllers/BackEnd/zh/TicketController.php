...
public function myTicketsDatatable()
{
    ...
    $user = CMS_Liveaccount::where('email',auth()->guard('admin')->user()->email)->first();
    ...
    return $datatables->editColumn('created_at',function($post){

      return Carbon::parse($post->created_at)->toDayDateTimeString();

  })->addColumn('details',function($post){

    return "<a href='/ticket-details-".$post->id."' target='blank'><button type='button' class='btn btn-success btn-min-width btn-round'>細節</button></a>";

})->raw(['details'])->make(true);
