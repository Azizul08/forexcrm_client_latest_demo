...
class CrmController extends Controller
{
  ...
  public function getIndex(Request $request){
    ...
    $accounts=DB::table('cms_account')->join('mt4_users','mt4_users.LOGIN','=','cms_account.account_no')
    ->where([
  ...
  // ORDERS (Total closed/running trades)
    ...
    foreach($all_accounts as $account){
      ...
      $run_trades=DB::table('mt4_trades')
      ->where([
        ['LOGIN',$account->account_no],
        ['CMD','0'],
        ['CLOSE_TIME','1970-01-01 00:00:00']
  ...
  // TOTAL Internal Transfer
    ...
    foreach($all_accounts as $account){
      ...
      $inter_trans=DB::table('cms_internal_transfer')
      ->where([
  ...
  */
  ...
  public function passwordVerification(Request $request){
    ...
    if($request->verification_code!=session('password_verification_code')){
...
public function openTradingAccount(){
  ...
  $cms_account=DB::table('cms_account_type')
  ->where([
    ['demo_live','live'],
    ['ac_name','Default'],
    ['status','=',1]
