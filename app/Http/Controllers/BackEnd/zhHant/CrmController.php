...
namespace App\Http\Controllers\BackEnd\zhHant;
...
public function postUpdateProfile(Request $request){
  if(strlen($request->mobile)<8){
    Session::flash('mobile_error','手機號碼太短');
...
public function postVerifyProfile(Request $request){
  ...
  if(!$request->address_proof && !$request->id_proof){
    return Redirect::back()->withErrors(['無法上載']);
  }
  ...
  Mail::send('backEnd.'.app()->getLocale().'.crm.mail.documents',
    ...
    {
      $message->from($from_mail,'用戶端設置驗證');
...
public function postChangePassword(Request $request){
  if($request->new_password!=$request->confirm_new_password){
    Session::flash('error','密碼不匹配。請重新輸入。');
    return Redirect::back();
  }
  else{
    $profile=DB::table('cms_liveaccount')->where('email',session('login_email'))->first();
...
public function passwordVerification(Request $request){if($request->verification_code!=session('password_verification_code')){
  return view('backEnd.'.app()->getLocale().'.crm.profile.password-verification',compact('request'))->withErrors(['驗證碼不正確。 ']);
}
...
public function postBankInformation(BankInformationRequest $request){
 ...
 Session::flash('success','銀行資料更新成功');
 return Redirect::back();         
}
...
$uppercase = preg_match('@[A-Z]@', $request->password);
  ...
  if(strlen($request->password) < 8) {
    Session::flash('error','密碼應至少包含8個字符');
    return Redirect::back()->withInput();
  }

  if(!$uppercase || !$lowercase || !$number){
    Session::flash('error','密碼應包含大寫、小寫和數字');
    return Redirect::back()->withInput();
  }

  if($request->password!=$request->c_password){
    Session::flash('error','密碼不匹配');
    return Redirect::back()->withInput();
  }
  $time = date("Y-m-d H:i:s");
    ...
    if($city=='' || $address=='' || $postal_code=='' || $state==''){
      Session::flash('profile-error','請填寫您的個人資料');
    ...
    if($total_accounts>=$allowance){
      Session::flash('max_allowance_error','您已排除此帳戶類型的交易帳戶的最大限額。');
      return Redirect::back();
    }
  ...
  $mail_subject=config('app.name').' 模擬帳戶詳細資料';
  $mail_data=$this->mail_data();
  $data=array(
    'date'=>$time,
    'name'=>$name,
...
$mail_subject=config('app.name').' 交易帳戶明細';
$mail_data=$this->mail_data();
$data=array(
  'date'=>$date_time,
  'name'=>$name,
...
public function postChangeLeverage(Request $request){
  ...
  if ($lev->leverage==$request->leverage) {
    return Redirect::back()->withErrors(['此帳戶已設定為 '.$request->leverage.':1']);
  }
  else{
    $profile=DB::table('cms_liveaccount')->where('email',session('login_email'))->first();
    $general_info=DB::table('general_information')->first();
...
public function leverageVerification(Request $request){
  if($request->verification_code!=session('leverage_verification_code')){
    return view('backEnd.'.app()->getLocale().'.crm.leverage.leverage-verification',compact('request'))->withErrors(['驗證碼不正確。 ']);
  }
    ...
    if($a!='Successful'){
      ...
      return view('backEnd.'.app()->getLocale().'.crm.leverage.leverage-verification',compact('request'))->withErrors(['杠杆現時無法發生變化。請稍後再試。']);
    }
...
public function mt4PasswordVerification(Request $request){
  if($request->verification_code!=session('mt4_password_verification_code')){
    return view('backEnd.'.app()->getLocale().'.crm.password.mt4-password-verification',compact('request'))->withErrors(['驗證碼不正確。 ']);
  }
    ...
    $mail_subject=config('app.name').' 交易帳戶密碼';
    $mail_data=$this->mail_data();
    $data=array(
...
public function postChangeMt4InvestorPassword(Request $request){
  ...
  if(strlen($request->investor_password) < 8) {
    Session::flash('investor_error','密碼應至少包含8個字符');
    return Redirect::back()->withInput();
  }

  if(!$uppercase || !$lowercase || !$number){
    Session::flash('investor_error','密碼應包含大寫、小寫和數字');
    return Redirect::back()->withInput();
  }

  if($request->investor_password!=$request->cinvestor_password){
    Session::flash('investor_error','密碼不匹配');
    return Redirect::back()->withInput();
  }
...
public function mt4InvestorPasswordVerification(Request $request){
  if($request->verification_code!=session('mt4_investor_password_verification_code')){
    return view('backEnd.'.app()->getLocale().'.crm.password.mt4-investor-password-verification',compact('request'))->withErrors(['驗證碼不正確。 ']);
  }
    ...
    $mail_subject=config('app.name').' 交易帳戶投資者密碼';
    $mail_data=$this->mail_data();
    $data=array(
...
public function postInternalTransfer(Request $request){
  ...
  if($running){
    if($running->CLOSE_TIME=="1970-01-01 00:00:00"){
      return redirect('/internal-transfer')->withErrors([$request->transfer_from.' 有一個或多個連續交易']);
    }
  }
  ...
  if($request->transfer_from==$request->transfer_to){
    return redirect('/internal-transfer')->withErrors(['僅轉帳至單獨帳戶。']);
  }
  elseif(is_numeric($request->amount)==false){
    return redirect('/internal-transfer')->withErrors(['請輸入有效金額']);
  }
  
  elseif($request->amount<1){
    return redirect('/internal-transfer')->withErrors(['最低內部轉帳金額為1美元']);
  }
  ...
  elseif($BALANCE>=$request->amount && $request->amount>=1){
    ...
    return redirect('/internal-transfer')->withErrors(['您的餘額不足以進行這次轉帳。']);
  }
...
public function postInternalVerification(Request $request){
  if($request->verification_code!=session('verification_code')){
    return view('backEnd.'.app()->getLocale().'.crm.withdraw.internal_verification_code',compact('request'))->withErrors(['驗證碼不正確。 ']);
  }
...
// Mail
  ...
  Session::flash('msg','您的內部轉帳已成功處理到 '.$request->transfer_to.' ('.$act2->act_type.')');
...
public function postVerification(Request $request){
  if($request->verification_code!=session('verification_code')){
    return view('backEnd.'.app()->getLocale().'.crm.withdraw.verification_code',compact('request'))->withErrors(['驗證碼不正確。 ']);
  }
...
public function postBankWithdrawFunds(BankInformationRequest $request){
  ...
  if(is_numeric($request->amount)==false){
    return redirect('/bank-withdraw-funds')->withErrors(['請輸入有效金額']);
  }
  
  elseif($request->amount<1){
    return redirect('/bank-withdraw-funds')->withErrors(['最低內部轉帳金額為1美元']);
  }
  elseif($BALANCE>=$request->amount && $request->amount>=1){
    $profile=DB::table('cms_liveaccount')->where('email',session('login_email'))->first();
    $name=$profile->fname.' '.$profile->lname;
    ...
    return redirect('/bank-withdraw-funds')->withErrors(['您的餘額不足以滿足這個取款請求']);
  }
