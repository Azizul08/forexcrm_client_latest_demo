...
use Datatables; 
use PDF;
use App\Http\Requests\BankInformationRequestDe;
...
public function ajaxLiveTrade(Request $request){
  ...
  if(count($trades)>0){
    ...
    echo '<tr><td colspan="14" style="text-align:center;"><strong>Kein laufender Handel</strong></td></tr>';
  } 
...
$profit=DB::table('mt4_trades')->where('mt4_trades.CMD','<','2')->where('mt4_trades.CLOSE_TIME','>=',$start)->where('mt4_trades.CLOSE_TIME','<=',$end)->where('mt4_trades.Login',$account_no)->sum('mt4_trades.PROFIT');
      ...
      echo '<b>Anzahlung:</b> '.round($deposit,2).', ';
      if($withdraw==0){
        echo '<b>Abheben:</b> 0, ';
      }
      else{
      echo '<b>Abheben:</b> '.(-1)*round($withdraw,2).', ';
      }
      echo '<b>Kredit:</b> '.round($creditIn,2).', ';
      echo '<b>Guthaben auszahlen:</b> '.round($creditOut,2).', ';
      echo '<b>Profitieren:</b> '.round($profit,2);
...
public function postUpdateProfile(Request $request){
  if(strlen($request->mobile)<8){
    Session::flash('mobile_error','Mobilfunknummer ist zu kurz');
  ...
  Session::flash('msg','Dein Profil wurde erfolgreich aktualisiert');
  return Redirect::back();
}
...
public function postVerifyProfile(Request $request){
  ...
  if(!$request->address_proof && !$request->id_proof){
    return Redirect::back()->withErrors(['Kann nicht hochgeladen werden']);
  }
  ...
  Session::flash('msg','Ihr Dokument wurde erfolgreich hochgeladen');
...
public function sendPasswordResetCode()
{
  ...
  $user_mail=session('login_email');
    // $user_mail='golamrobbani021@gmail.com';
...
public function postChangePassword(Request $request){
  if($request->new_password!=$request->confirm_new_password){
    Session::flash('error','Passwort stimmt nicht überein. Bitte erneut eingeben.');
    return Redirect::back();
  }
  else{
    $profile=DB::table('cms_liveaccount')->where('email',session('login_email'))->first();
...
public function passwordVerification(Request $request){if($request->verification_code!=session('password_verification_code')){
  return view('backEnd.'.app()->getLocale().'.crm.profile.password-verification',compact('request'))->withErrors(['Der Bestätigungscode ist falsch.']);
}
  ...
  Session::flash('msg','Ihr Passwort wurde erfolgreich geändert');
...
public function postBankInformation(BankInformationRequestDe $request){
 DB::table('cms_liveaccount')
 ->where('email', session('login_email'))
 ->update([
 ...
 Session::flash('success','Bankinformationen wurden erfolgreich aktualisiert');
 return Redirect::back();         
}
...
$uppercase = preg_match('@[A-Z]@', $request->password);
  ...
  if(strlen($request->password) < 8) {
    Session::flash('error','Das Passwort sollte mindestens 8 Zeichen enthalten');
    return Redirect::back()->withInput();
  }

  if(!$uppercase || !$lowercase || !$number){
    Session::flash('error','Das Passwort sollte Großbuchstaben, Kleinbuchstaben und Zahlen enthalten');
    return Redirect::back()->withInput();
  }

  if($request->password!=$request->c_password){
    Session::flash('error','Passwort stimmt nicht überein');
    return Redirect::back()->withInput();
  }
  $time = date("Y-m-d H:i:s");
    ...
    if($city=='' || $address=='' || $postal_code=='' || $state==''){
      Session::flash('profile-error','Bitte füllen Sie Ihre Profilinformationen aus');
    ...
    if($total_accounts>=$allowance){
      Session::flash('max_allowance_error','Sie haben die maximale Anzahl von Handelskonten für diesen Kontotyp überschritten.');
      return Redirect::back();
    }
  ...
  $mail_subject=config('app.name').' Demo-Konto Details';
  $mail_data=$this->mail_data();
  $data=array(
    'date'=>$time,
    'name'=>$name,
...
$mail_subject=config('app.name').' Handelskonto Details';
$mail_data=$this->mail_data();
$data=array(
  'date'=>$date_time,
  'name'=>$name,
...
public function postChangeLeverage(Request $request){
  ...
  if ($lev->leverage==$request->leverage) {
    return Redirect::back()->withErrors(['Dieses Konto ist bereits auf'.$request->leverage.':1 festgelegt']);
  }
    ...
    $from_name=config('app.name');
    $user_mail=session('login_email');
    $mail_subject='Bestätigungscode für Hebelwirkung';
    $verification_code=mt_rand(100000, 999999);
...
public function leverageVerification(Request $request){
  if($request->verification_code!=session('leverage_verification_code')){
    return view('backEnd.'.app()->getLocale().'.crm.leverage.leverage-verification',compact('request'))->withErrors(['Der Bestätigungscode ist falsch.']);
  }
    ...
    if($a!='Successful'){
      ...
      return view('backEnd.'.app()->getLocale().'.crm.leverage.leverage-verification',compact('request'))->withErrors(['Hebelwirkung ist für diesen Moment nicht möglich. Bitte versuche es später erneut.']);
    }
    ...
    Session::flash('msg','Die Hebelwirkung wurde erfolgreich geändert');
...
public function postChangeMt4Password(Request $request){
  ...
  $user_mail=session('login_email');
  // $user_mail='mkhassan25@gmail.com';
  $mail_subject='Bestätigungscode für MT4 Passwortänderung';
  $verification_code=mt_rand(100000, 999999);
...
public function mt4PasswordVerification(Request $request){
  if($request->verification_code!=session('mt4_password_verification_code')){
    return view('backEnd.'.app()->getLocale().'.crm.password.mt4-password-verification',compact('request'))->withErrors(['Der Bestätigungscode ist falsch.']);
  }
    ...
    $mail_subject=config('app.name').' Handelskontokennwort';
    $mail_data=$this->mail_data();
    $data=array(
...
public function postChangeMt4InvestorPassword(Request $request){
  ...
  if(strlen($request->investor_password) < 8) {
    Session::flash('investor_error','Das Passwort sollte mindestens 8 Zeichen enthalten');
    return Redirect::back()->withInput();
  }

  if(!$uppercase || !$lowercase || !$number){
    Session::flash('investor_error','Das Passwort sollte Großbuchstaben, Kleinbuchstaben und Zahlen enthalten');
    return Redirect::back()->withInput();
  }

  if($request->investor_password!=$request->cinvestor_password){
    Session::flash('investor_error','Passwort stimmt nicht überein');
    return Redirect::back()->withInput();
  }
...
public function mt4InvestorPasswordVerification(Request $request){
  if($request->verification_code!=session('mt4_investor_password_verification_code')){
    return view('backEnd.'.app()->getLocale().'.crm.password.mt4-investor-password-verification',compact('request'))->withErrors(['Der Bestätigungscode ist falsch.']);
  }
    ...
    $mail_subject=config('app.name').' Handelskonto-Investor-Passwort';
    $mail_data=$this->mail_data();
    $data=array(
...
public function postInternalTransfer(Request $request){
  ...
  if($running){
    if($running->CLOSE_TIME=="1970-01-01 00:00:00"){
      return redirect('/internal-transfer')->withErrors([$request->transfer_from.' hat einen oder mehrere laufende Trades']);
    }
  }
  ...
  if($request->transfer_from==$request->transfer_to){
    return redirect('/internal-transfer')->withErrors(['Übertragen Sie nur auf ein separates Konto.']);
  }
  elseif(is_numeric($request->amount)==false){
    return redirect('/internal-transfer')->withErrors(['Bitte geben Sie einen gültigen Betrag ein']);
  }
  
  elseif($request->amount<1){
    return redirect('/internal-transfer')->withErrors(['Minimaler interner Transfer ist 1 usd']);
  }
  ...
  elseif($BALANCE>=$request->amount && $request->amount>=1){
    ...
    $user_mail=session('login_email');
    $mail_subject='Verifizierungscode für die interne Übertragung';
    $verification_code=mt_rand(100000, 999999);
    ...
    return redirect('/internal-transfer')->withErrors(['Ihr Guthaben reicht für diese Überweisung nicht aus']);
  }
...
public function postInternalVerification(Request $request){
  if($request->verification_code!=session('verification_code')){
    return view('backEnd.'.app()->getLocale().'.crm.withdraw.internal_verification_code',compact('request'))->withErrors(['Der Bestätigungscode ist falsch.']);
  }
...
// Mail
  ...
  Session::flash('msg','Ihre interne Übertragung wurde erfolgreich zu '.$request->transfer_to.' ('.$act2->act_type.') verarbeitet');
...
public function postNetellerWithdraw(Request $request){
  ...
  if(is_numeric($request->amount)==false){
    return redirect('/neteller_withdraw')->withErrors(['Bitte geben Sie einen gültigen Betrag ein']);
  }
...
public function postPerfectMoneyWithdraw(Request $request){
  ...
  if(is_numeric($request->amount)==false){
    return redirect('/perfect_money_withdraw')->withErrors(['Bitte geben Sie einen gültigen Betrag ein']);
  }
...
public function skrillWithdraw(Request $request){
  ...
  $accounts = $accounts = DB::table('cms_account')->join('mt4_users','mt4_users.LOGIN','=','cms_account.account_no')->select(['cms_account.account_no','mt4_users.BALANCE as balance',DB::raw('CONCAT(cms_account.account_no," ( ",cms_account.act_type," )") as acc_no')])->where([['cms_account.act_type','<>','DEMO'],['cms_account.act_type','<>','NULL'],['cms_account.email','=',session('login_email')]])->orderBy('cms_account.account_no','desc')->get();
...
public function postSkrillWithdraw(Request $request){
  ...
  if(is_numeric($request->amount)==false){
    return redirect('/skrill_withdraw')->withErrors(['Bitte geben Sie einen gültigen Betrag ein']);
  }
...
public function postOkpayWithdraw(Request $request){
  ...
  if(is_numeric($request->amount)==false){
    return redirect('/okpay_withdraw')->withErrors(['Bitte geben Sie einen gültigen Betrag ein']);
  }
...
public function postVerification(Request $request){
  if($request->verification_code!=session('verification_code')){
    return view('backEnd.'.app()->getLocale().'.crm.withdraw.verification_code',compact('request'))->withErrors(['Der Bestätigungscode ist falsch.']);
  }
...
public function postBankWithdrawFunds(BankInformationRequestDe $request){
  ...
  if(is_numeric($request->amount)==false){
    return redirect('/bank-withdraw-funds')->withErrors(['Bitte geben Sie einen gültigen Betrag ein']);
  }
  
  elseif($request->amount<1){
    return redirect('/bank-withdraw-funds')->withErrors(['Mindestüberweisung ist 1 usd']);
  }
  elseif($BALANCE>=$request->amount && $request->amount>=1){
    $profile=DB::table('cms_liveaccount')->where('email',session('login_email'))->first();
    $name=$profile->fname.' '.$profile->lname;
    ...
    $from_name=config('app.name');
    $user_mail=session('login_email');
    $mail_subject='Bestätigungscode für die Auszahlung von Fonds';
    $verification_code=mt_rand(100000, 999999);
    session(['verification_code' => $verification_code]);
    Session::save();
    $data = array('name' => $name , 'verification_code' => session('verification_code'));
    ...
    return redirect('/bank-withdraw-funds')->withErrors(['Ihr Kontostand ist für diese Auszahlungsanforderung nicht ausreichend']);
  }
